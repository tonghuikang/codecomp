...
def process(string_input):
    ...
    d = {}
    assgn = {}
    for rule in string_input.split("\n"):
        char = rule.split(":")[0]
        phrase = rule.split(":")[1].strip()
        # log(phrase, phrase[0])
        if phrase[0] != '"':
            deps = [[int(x) for x in seg.strip().split()] 
                    for seg in phrase.split("|")]
            d[int(char)] = deps
        else:
            assgn[int(char)] = phrase[1:-1]
...
d, assgn = process(sample_input1)

sample_res = solve(d, assgn, sample_input2)
print(sample_res)
...
d, assgn = process(test_input1)
test_res = solve(d, assgn, test_input2)
print(test_res)

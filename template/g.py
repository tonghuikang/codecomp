...
def solve_(n,arr):
    ...
    pyramid_value = 0  # the result is the bitcount
    ...
    for i,x in enumerate(arr):
        if x < len(pyramid) and pyramid[x] < allowance(x):
            pyramid[x] += 1
            pyramid_value += (1 << x)
...
for case_num in range(int(input())):
    ...
    arr = [n+2 if x > n+1 else x for x in arr]
    # arr = minus_one(arr)

...
def solve_(srr, mrr):
    ...
    # for no ABC in subsequence
        # no A, B or C
        # B is not in between A and C
            # no C after the last B
            # no A before the first B

    # proposed
        # number of B
        # number of C after the last B
        # number of A before the first B

    srr = list(srr)

    g = {}
    for abc in "abc":
        g[abc] = SortedList([i for i,x in enumerate(srr) if x == abc])

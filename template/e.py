...
def solve_(n, arr):
    ...
    # hao removes first
        # wants to remove late and large
        # wants to remove early and small
    # alex then locks
        # wants to lock late and large
        # wants to lock early and small

    # why is this difficult?
    # needs to be larger index minus smallest index

    # if n is small, just brute force

    # for each element, at least one of the later two
    # for every (A B) (C D), you can guarantee C - B
    # for every partition - second largest on right - second smallest on left

    # only the first 4 steps matter?

    # lower bound, alex can guarantee this
    left = SortedList([(x,i) for i,x in enumerate(arr[:1])])
    right = SortedList([(x,i) for i,x in enumerate(arr[1:], start=1)])
    ...
    for i,x in enumerate(arr[1:-2], start=1):
        transfer = (x,i)
        left.add(transfer)
        right.remove(transfer)

...
def solve_(mrr,n,k):
    ...
    # start at max, win
    # start more than k from max, lose
    # within diamond around max, if no element k larger from you, you win
    # maximum 5 candidates it seems (make it 8)
    ...
    def slider(mrr):
        ...
        all_nums = SortedList([-1])
        excl_nums = SortedList([-1])

        for i in range(n):
            for j in range(n):
                windows[i+j].append(mrr[i][j])

        log(windows)

        for ij in range(k+1):
            lst = windows[ij]
            excl_nums.update(lst)

        for ij in range(k+1, 2*n+2):
            lst = windows[ij]
            all_nums.update(lst)
        ...
        for ij in range(2*n):
            ...
            for i in range(ij + 1):
                j = ij - i
                if 0 <= i < n and 0 <= j < n:
                    if mrr[i][j] < curmax and mrr[i][j] > 0:
                        removing.add((i,j))

            entering = windows[ij - k]
            exiting = windows[ij + k+1]
    ...
    mrr = slider(mrr)
    log(mrr)
    mrr = [col[::-1] for col in zip(*mrr)]  # once
...
# for case_num in range(int(input())):
    ...
    res = "\n".join("".join("M" if x > 0 else "G" for x in row) for row in res)

...
def solve_(srr, trr, k):
    ...
    psrr = {abc:deque() for abc in range(26)}
    # psrr = defaultdict(lambda: collections.deque([]))
    ...
    cur = 0
    for i,ref in enumerate(trr):

        # get a smaller element to the left
        minleft = MAXINT
        for abc in range(26):
            if abc >= ref:
                break
        ...
        # get an equal element to the left and repeat
        abc = ref
        while psrr[abc]:
            original_idx = psrr[abc][0]
            if (abc + 1000*original_idx) not in sl:

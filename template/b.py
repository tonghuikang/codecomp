...
def solve_(arr, brr, n, k):
    ...
    health_pool = SortedList()
    power_pool = SortedList()
    ...
    while health_pool:
        if current_attack <= 0:
            return no
        smallest_health, _ = health_pool[0]
        health_threshold += current_attack

        while health_pool and health_pool[0][0] <= health_threshold:
            _, idx = health_pool[0]
            del health_pool[0]

            power_removed = brr[idx]
            power_pool.remove(power_removed)

        if not health_pool:
            return yes

        weakest_power = power_pool[0]
        current_attack -= weakest_power

        ...
        for i,plate in enumerate(field[1:-1], start=1):
            for j,row in enumerate(plate[1:-1], start=1):
                for k,cell in enumerate(row[1:-1], start=1):
                    for l,_ in enumerate(row[1:-1], start=1):
                        count = 0
                        for x in [-1,0,1]:
                            for y in [-1,0,1]:
                                for z in [-1,0,1]:
                        ...
                        if field[i][j][k][l] == 1:
                            ...
                            if count == 3:
                                new_field[i][j][k][l] = 1
        totals = sum(sum(sum(sum(cell) for cell in row) for row in plate) for plate in new_field)
        log(totals)
        field = new_field

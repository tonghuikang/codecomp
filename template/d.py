...
def solve_(n,m,arr,srr,drr):
    # your solution here

    # auction the slots to the most common remaining color
    ...
    # might need to sort
    for s,d in zip(srr, drr):
        vals = [0 for _ in range(d)]
        # log(vals)
        cur = 0
        for _ in range(s):
            vals[cur] += 1
    ...
    for i,vals in enumerate(shelves):
        # largest space to the most common color
        vals = SortedList(vals)

        re_add = []

        while vals:
            if not freq_and_color:
                print(-1)
                return
            freq, color = freq_and_color[-1]
            del freq_and_color[-1]
            space = vals[-1]
            del vals[-1]

            populated = min(space, freq)
            ...
            freq -= populated
            if freq > 0:
                re_add.append((freq, color))
            
            space -= populated
            if space > 0:
                vals.add(space)
        
        for freq, color in re_add:
            freq_and_color.add((freq, color))
...
for case_num in range(int(input())):
    ...
    res = solve(n,m,arr,srr,drr)  # include input here

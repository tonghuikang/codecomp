...
def solve_(n,m,mrr):
    ...
    def compute(mrr):
        max_complete_left = 0
        max_incomplete_left = 0
        incomplete_left_by_end = SortedList([])  # end, start
        incomplete_left_by_start = SortedList([])  # end, start

        for a,b in mrr:
            while incomplete_left_by_end and incomplete_left_by_end[0][0] < a:
                xb,xa = incomplete_left_by_end[0]
                del incomplete_left_by_end[0]
                incomplete_left_by_start.remove((xa,xb))
                dist = xb-xa+1
                max_complete_left = max(max_complete_left, dist)

            if incomplete_left_by_start:
                incomplete_left = a-incomplete_left_by_start[0][0]
                max_incomplete_left = max(max_incomplete_left, incomplete_left)

            incomplete_left_by_end.add((b,a))
            incomplete_left_by_start.add((a,b))
        
        return max(max_complete_left, max_incomplete_left)

    res = compute(mrr)
    maxres = max(maxres, res)

    mrr = [[-b,-a] for a,b in mrr]
    mrr.reverse()

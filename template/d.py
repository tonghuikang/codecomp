...
def solve_(arr):
    ...
    xrr = [x-i for i,x in enumerate(arr)]  # offset = i
    yrr = [x+i for i,x in enumerate(arr)]  # offset = -i

    # log(xrr)
    # log(yrr)

    left_pool = SortedList([])
    right_pool = SortedList(xrr)
    ...
    for i,x in enumerate(arr):
        ...
        right_pool.remove(right_to_remove)

        if left_pool and right_pool:
            # log(left_pool, x, right_pool)
            res = left_pool[-1] + right_pool[-1] + x
            maxres = max(maxres, res)

        left_pool.add(left_to_add)

...
def solve_(n, m, arr, brr, crr):
    # your solution here

    # iterate over number of can openers to use

    holding = SortedList([])
    holding_sum = 0
    maxres = 0

    while len(holding) < m and arr:
        x = arr.pop()
        holding.add(x)
        holding_sum += x
        maxres = max(maxres, holding_sum)
...
# for case_num in range(int(input())):
    ...
    res = solve(n, m, arr, brr, crr)  # include input here

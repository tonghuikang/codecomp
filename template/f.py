...
def solve_(arr):
    ...
    #     pts_by_heights.append((h,x))

    # pts_by_heights.sort()
    # pts_by_heights.reverse()

    all_heights = sorted(set(arr))
    ...
    for height in all_heights:
        ...
        for x in arr:
            if x >= height:
                ...
                cur = 0
        maxwidth_by_heights[height] = maxlen


    log(maxwidth_by_heights)

    maxres = 0
    for i,x in enumerate(all_heights):
        res = maxwidth_by_heights[x]*x
        maxres = max(maxres, res)

    for i,x in enumerate(all_heights):
        for y in all_heights[i+1:]:
            res = maxwidth_by_heights[x]*x + maxwidth_by_heights[y]*(y-x)
            log(res, maxwidth_by_heights[x]*x, maxwidth_by_heights[y]*(y-x))
            maxres = max(maxres, res)
